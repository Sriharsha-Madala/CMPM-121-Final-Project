Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/1fb61b927dcae0644b7c158be6be5b40"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			{
				#if defined(PROP_ANISOCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				float4 specMap = POI2D_SAMPLER_PAN(_AnisoColorMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float4 specMap = float4(1, 1, 1, 0);
				#endif
				float3 binormalObj = normalize(mul(unity_WorldToObject, poiMesh.binormal));
				float shadowMask = lerp(1, poiMax(poiLight.rampedLightMap), float(0.874));
				#ifdef POI_PASS_ADD
				shadowMask *= poiLight.attenuation;
				#endif
				float spec0 = calculateAnisotropics(binormalObj, float(4.9) +float(0) * specMap.a, poiMesh.normals[1], poiCam.viewDir, poiLight.direction, float(0) * 1000, float(1), shadowMask);
				float spec1 = calculateAnisotropics(binormalObj, float(0) +float(0) * specMap.a, poiMesh.normals[1], poiCam.viewDir, poiLight.direction, float(0) * 1000, float(1), shadowMask);
				spec0 = lerp(spec0, aaEdgeFeather(spec0, float(0.5), float(0)), float(0));
				spec1 = lerp(spec1, aaEdgeFeather(spec1, float(0.5), float(0)), float(0));
				float3 spec0Color = specMap.rgb * float4(1,1,1,1);
				float3 spec1Color = specMap.rgb * float4(1,1,1,1);
				float3 finalSpec = saturate(saturate(spec0 * spec0Color) + saturate(spec1 * spec1Color)) * lerp(1, poiFragData.baseColor, float(0)) * lerp(dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), poiLight.directColor, float(1));
				float3 baseColor = poiFragData.baseColor;
				poiFragData.baseColor = lerp(poiFragData.baseColor, spec1Color * lerp(1, baseColor, float(0)) * lerp(dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), poiLight.directColor, float(1)), float(0) * spec1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, spec0Color * lerp(1, baseColor, float(0)) * lerp(dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), poiLight.directColor, float(1)), float(0) * spec0);
				poiLight.finalLightAdd += max(0, finalSpec * float(1));
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					float vSpec0 = calculateAnisotropics(binormalObj, float(4.9) +float(0) * specMap.a, poiMesh.normals[1], poiCam.viewDir, poiLight.vDirection[index], float(0) * 1000, float(1), poiLight.vAttenuationDotNL[index]);
					float vSpec1 = calculateAnisotropics(binormalObj, float(0) +float(0) * specMap.a, poiMesh.normals[1], poiCam.viewDir, poiLight.vDirection[index], float(0) * 1000, float(1), poiLight.vAttenuationDotNL[index]);
					vSpec0 = lerp(vSpec0, aaEdgeFeather(vSpec0, float(0.5), float(0)), float(0));
					vSpec1 = lerp(vSpec1, aaEdgeFeather(vSpec1, float(0.5), float(0)), float(0));
					float3 vSpec0Color = specMap.rgb * float4(1,1,1,1);
					float3 vSpec1Color = specMap.rgb * float4(1,1,1,1);
					poiLight.finalLightAdd += max(0, saturate(saturate(vSpec0 * spec0Color) + saturate(vSpec1 * spec1Color)) * lerp(1, poiFragData.baseColor, float(0)) * lerp(dot(poiLight.vColor[index], float3(0.299, 0.587, 0.114)), poiLight.vColor[index], float(1)) * float(1));
					poiFragData.baseColor = lerp(poiFragData.baseColor, spec1Color * lerp(1, baseColor, float(0)) * lerp(dot(poiLight.vColor[index], float3(0.299, 0.587, 0.114)), poiLight.vColor[index], float(1)), float(0) * vSpec1);
					poiFragData.baseColor = lerp(poiFragData.baseColor, spec0Color * lerp(1, baseColor, float(0)) * lerp(dot(poiLight.vColor[index], float3(0.299, 0.587, 0.114)), poiLight.vColor[index], float(1)), float(0) * vSpec0);
				}
				#endif
				return float3(0, 0, 0);
			}
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(4) != POI_MODE_TRANSPARENT)
				{
